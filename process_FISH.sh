...
conda run --live-stream -n merfish3d_052926  bash -lc "qi2lab-preprocess $path_to_data $preprocess_opts"
echo "Finished registration for $path_to_data"


echo "Starting global registration"
conda run --live-stream -n merfish3d_052926 bash -lc "qi2lab-globalregister $path_to_data $global_register_opts"
echo "Finished global registration for $path_to_data"
 

echo "Starting segmentation"
conda run --live-stream -n merfish3d_052926 bash -lc "qi2lab-segment $path_to_data $segment_opts"
echo "Finished segmentation for $path_to_data"

if [[ "$is_MERFISH" == true ]]; then
    echo "Starting decoding"
    conda run --live-stream -n merfish3d_052926 bash -lc "qi2lab-decode $path_to_data $decode_opts"
