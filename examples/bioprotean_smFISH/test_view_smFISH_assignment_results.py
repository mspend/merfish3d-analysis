...
# Define a color map for genes
gene_colors = {
    'Aif1': 'cyan',
    'Itgam': 'magenta',
    # Add more genes and their colors as needed
}
...
for bit_file in sorted(decoded_spots_path.glob('bit*.csv')):
   ...
   color = gene_colors.get(gene_name, 'white')  # Default to white if gene is not in the color map
   viewer.add_points(pts,name=gene_name,size=1, face_color="white")
